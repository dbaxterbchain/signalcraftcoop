...
- Prod API now via Lambda HTTP API (execute-api URL; custom domain pending).
...
- API Gateway custom domain for Lambda HTTP API (e.g., `api.signalcraftcoop.com`).
...
- Prod endpoints: app `https://app.signalcraftcoop.com`, api (temporary) `https://bkq3n9fjr2.execute-api.us-west-2.amazonaws.com`.
